...
import sidenavSource from '../../components/Sidenav/Sidenav.tsx?raw';
import { sourceDocs } from '../utils/sourceDocs';

interface NavItemData {
  label: string;
  faCode: string;
  state?: 'active' | 'active2';
  arrow?: boolean;
  children?: { label: string; faCode: string }[];
}

const navItems: NavItemData[] = [
  { label: 'Overview', faCode: 'house', state: 'active2' },
  { label: 'Messages', faCode: 'envelope' },
  { label: 'Groups', faCode: 'people-group' },
  { label: 'Challenges', faCode: 'trophy' },
  { label: 'Clients', faCode: 'users' },
  {
    label: 'Workouts & Programs', faCode: 'dumbbell', arrow: true,
    children: [
      { label: 'AI Workout Builder', faCode: 'wand-magic-sparkles' },
      { label: 'Programs', faCode: 'clipboard-list' },
      { label: 'Workouts', faCode: 'dumbbell' },
      { label: 'Exercises', faCode: 'person-running' }
    ]
  },
  {
    label: 'Nutrition', faCode: 'bowl-food', arrow: true,
    children: [
      { label: 'Meal Plans', faCode: 'utensils' },
      { label: 'Recipes', faCode: 'book-open' }
    ]
  },
  { label: 'Habits', faCode: 'person-arrow-up-from-line' },
  {
    label: 'Booking & Events', faCode: 'calendar-days', arrow: true,
    children: [
      { label: 'Calendar', faCode: 'calendar' },
      { label: 'Events', faCode: 'calendar-star' }
    ]
  },
  {
    label: 'Business', faCode: 'chart-mixed', arrow: true,
    children: [
      { label: 'Payments', faCode: 'credit-card' },
      { label: 'Reports', faCode: 'chart-line' }
    ]
  },
  {
    label: 'Settings', faCode: 'gear', arrow: true,
    children: [
      { label: 'Account', faCode: 'user' },
      { label: 'Billing', faCode: 'file-invoice-dollar' },
      { label: 'Integrations', faCode: 'plug' }
    ]
  }
];

const meta: Meta<typeof Sidenav> = {
  title: 'Experiment/Sidenav 2.0',
  component: Sidenav,
  subcomponents: {
    Main: Sidenav.Main,
  ...
  return (
    <div className="sidenav-trainer__logo" aria-label="GO Fitness">
      <svg width="134" height="21" viewBox="0 0 134 21" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g clipPath="url(#sidenav2-logo-clip)">
        ...
        <defs>
          <clipPath id="sidenav2-logo-clip">
...
function NavMenu({ layout = 'full' as 'full' | 'mini', showQuickAdd = false }) {
  const isFull = layout === 'full';
  return (
    <div className="sidenav-main__items">
      {showQuickAdd && (
        <Sidenav.NavItem
          label="Quick add"
          faCode="plus"
      ...
      {navItems.map((item) => {
        if (item.children && isFull) {
          return (
            <div key={item.label} className="sidenav-flyout">
              <Sidenav.NavItem
                label={item.label}
                faCode={item.faCode}
                ...
                arrow
              />
              <div className="sidenav-flyout__panel">
                <Sidenav.Group title={item.label}>
...
export const Playground: Story = {
  ...
  render: (args) => {
    const isFull = (args.layout ?? 'full') === 'full';
    return (
      <Sidenav {...args}>
        <TrainerLogo />
        ...
        <div className="sidenav-trainer__search">
          ...
          {isFull && (
            <span className="sidenav-trainer__search-label">Search</span>
          )}
        </div>

        <NavMenu layout={args.layout ?? 'full'} showQuickAdd />
      </Sidenav>
    );
  }
};
